...
另外一种退出方式是在方法执行的过程中遇到了异常，并且这个异常没有在方法体内得到妥善处理。无论是 Java 虚拟机内部产生的异常，还是代码中使用 athrow 字节码指令产生的异常，只要在本方法的异常表中没有搜索到匹配的异常处理器，就会导致方法退出，这种退出方法的方式称为“异常调用完成“。
...
**静态分派**：**依赖静态类型来决定方法执行版本的分派动作，称为静态分派**

- **重载**：原理是静态分派，虚拟机在重载时通过**参数的静态类型**而不是实际类型作为判定依据。
...
- **动态分派**对应着Java的方法的**重写**
- **动态分派**：**运行期**根据**实际类型**来确定方法的选择，来判断定位方法执行版本的分派过程。
- **重写的本质**：运行期方法调用机制 会将常量池中的类方法**符号引用**解析到不同的**直接引用**上。
- 原理：

> 1. 是根据`虚方法表`
> 2. 在类加载时的链接阶段被创建并开始初始化
> 3. **每一个类都存在一个虚方法表**，虚方法表存放着各个方法的实际入口地址。
> 4. 如果某个方法在子类中没有重写，则子类的虚方法表中的地址入口和父类相同方法的地址入口一致
> 5. 如果子类重写了这个方法，子类虚方法表中的地址就会被替换指向子类的实现版本的入口地址。
